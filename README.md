...
>
>Формула алгоритма **kNN** (*k* ближайших соседей) выглядит: $$a(u,x^l,k) = arg \max\limits_{y \in Y} \sum\limits^k_{i=1}[y_u^i = y] ~~ где ~~ y_u^i = y^*(x_u^i) - ответ~~на~~i-том~~соседе$$
>
...
>---
>## kWNN
>Формула алгоритма ***kWNN*** (*k* взвешаных ближайших соседей) выглядит: $a(u,x^l,k) = arg \max\limits_{y \in Y} \sum\limits^k_{i=1}[y_u^i = y]w(i) $
>Возьмем за вес $w(i) = q^i$ и переберем по LOO
>
>
...
>#### Прямоугольное ядро
>
> Формула: $RectKer(r) = \dfrac{1}{2}[|r \leqslant 1|]$
>
...
>#### Треугольное ядро
>
> Формула: $TriaKer(r) = (1 - |r|) \cdot [|r \leqslant 1|]$
>
> Функция ядра на языке **R**:
...
>#### Квартическое  ядро
>
> Формула: $QuarKer(r) = (1 - r^2)^2 \cdot [|r \leqslant 1|]$
>
> Функция ядра на языке **R**:
...
>#### Ядро Епанечникова
>
> Формула: $EpanKer(r) = (1 - r^2) \cdot [|r \leqslant 1|]$
>
> Функция ядра на языке **R**:
...
> $$W_y(i,u) = \gamma_i \cdot K(\dfrac{1}{h_i}p(u,x_u^{(i)})),~~где~~\gamma_i \geqslant 0, h_i \geqslant 0, K(r) - функция ~~ ядра$$
