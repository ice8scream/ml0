...
LOO <- function(x, kerF, kerN="") {
    ...
    plot(params, mark, type="l",xlab="Řčđčíŕ îęíŕ", ylab="Îöĺíęŕ", main=paste("LOO для парзеновского окна с ", kerN))
...
PlotMapParsen <- function(h = 0.1, kerF = GausKer, label="Гауссовским") {
...
# dev.off()
# svg('GausKerLOO.svg'))
#     LOO(iris[,3:5],GausKer,"Гауссовским");
# dev.off()
